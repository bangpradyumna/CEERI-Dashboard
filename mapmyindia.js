/*1.3*/ var mmi = [
  ".js",
  "fxg5xxjyla88nsuwhengavq9roz2lred",
  ".css",
  "mapmyindia_v2.1",
  "https://apis.mapmyindia.com/map_v3/v1.3//",
  "Traffic,MultiLang,Satellite,MapSearch",
  "1",
  "1",
  "",
  "",
...
css.setAttribute(
  "href",
  "https://apis.mapmyindia.com/map_v3/v1.3/" + mmi[3] + mmi[2]
);
document
  .getElementsByTagName("head")[0]
  .appendChild(css); /* leaflet v 1.3.4++ */
!(function(t, i) {
  ...
  var Se = Te.extend({
    ...
    Ze = Te.extend({
      options: { position: "bottomright", prefix: '<div id="attr"></div>' },
      initialize: function(t) {
...
/*function*/ var map_o = [],
  ...
  satellite = "";
var MapmyIndia = {
  s_icn: "https://maps.mapmyindia.com/images/srarch1.png",
  num: 0,
  Map: function(id, params) {
    var tileUrl = "https://{s}.mapmyindia.com";
    if (params.tileUrl) tileUrl = params.tileUrl;
    if (params.center) center = params.center;
    else center = [28.61, 77.23];
    if (params.zoomControl === false) zc = false;
    else zc = true;
    if (params.indic === true) indic = 1;
    if (params.hybrid === true || params.satellite === true) mhy = true;
    else mhy = false;
    if (params.zoom) zm = params.zoom;
    else zm = 15;
    if (params.minZoom) mizm = params.minZoom;
    else mizm = mmi[9] ? mmi[9] : 4;
    if (params.maxZoom) mxzm = params.maxZoom;
    else mxzm = mmi[10] ? mmi[10] : 18;
    var defTile = "dft";
    if (params.defaultTile) {
      if (params.defaultTile == "indic") defTile = "ind";
      else if (params.defaultTile == "hybrid") defTile = "h";
      else if (params.defaultTile == "regional") defTile = "reg";
    }
    ...
    if (mmi[8] || params.maxBounds) {
      var bnd = mmi[8] ? eval(mmi[8]) : params.maxBounds;
      params["maxBounds"] = L.latLngBounds(L.latLng(bnd[0]), L.latLng(bnd[1]));
    }
    try {
      map_o[MapmyIndia.num] = new L.Map(id, params).setView(center, zm);
    } catch (e) {
      console.log(e);
      return false;
    }
    map_o[MapmyIndia.num].on("contextmenu", function(e) {
      L.DomEvent.preventDefault(event);
    });
    if (map_o[MapmyIndia.num].zoomControl)
      map_o[MapmyIndia.num].removeControl(map_o[MapmyIndia.num].zoomControl);
    L.control.scale({ metric: true }).addTo(map_o[MapmyIndia.num]);
    var element = L.DomUtil.create("div", "m_ctr" + MapmyIndia.num);
    var innerHTML = '<div class="map-control"><ul class="expand-map-control">';
    if (zc !== false)
      innerHTML +=
        '<li style="border-bottom: 1px solid #9e8d8d;"><a alt="Zoom In" title="Zoom In" class="map-zoom-in" onclick="map_o[' +
        MapmyIndia.num +
        '].zoomIn();return false;">+</a></li><li style="border-bottom: 5px solid transparent"><a alt="Zoom Out" title="Zoom Out" class="map-zoom-out" onclick="map_o[' +
        MapmyIndia.num +
        '].zoomOut()" style="line-height:25px;font-size:40px">-</a></li>';
    if (params.location !== false)
      innerHTML +=
        '<li onclick="MapmyIndia.geo(map_o[' +
        MapmyIndia.num +
        '])" style="border-bottom: 1px solid #9e8d8d;"><a id="geo_location" alt="Current Location" title="Current Location" class="on"><i class="ti ti-target"></i></a></li>';
    /*if(mmi[7]==1) indic='<li class="layer_tab" id="ind_t'+MapmyIndia.num+'" onclick=" MapmyIndia.tls(map_o['+MapmyIndia.num+'],'+MapmyIndia.num+',\'ind\',\''+tileUrl+'\')" ><a><span class="layer-item-img"><img src="https://maps.mapmyindia.com/images/layer_panel/indic_map.png" style="width:50px !important;"></span>Indic</a></li>';*/ if (
      mmi[5].search("MultiLang") != -1
    )
      satellite =
        '<li class="layer_tab" id="reg_t' +
        MapmyIndia.num +
        '" onclick="MapmyIndia.tls(map_o[' +
        MapmyIndia.num +
        "]," +
        MapmyIndia.num +
        ",'reg','" +
        tileUrl +
        '\')" ><a><span class="layer-item-img"><img src="' +
        mmi[4].slice(0, -1) +
        '/regional.png" style="width:50px !important"></span>Regional</a></li>';
    if (mmi[5].search("Satellite") != -1 && mhy != false)
      satellite +=
        '<li class="layer_tab" id="h_t' +
        MapmyIndia.num +
        '" onclick=" MapmyIndia.tls(map_o[' +
        MapmyIndia.num +
        "]," +
        ...
        ",'h','" +
        tileUrl +
        '\')"><a><span class="layer-item-img"><img src="https://maps.mapmyindia.com/images/hybridview.jpg" style="width:50px !important;"></span>Satellite</a></li>';
    /*if(params.traffic!==false) */ innerHTML +=
      '<li class="layer-item-wrap" style="' +
      (!mmi[5] || mmi[5] == "MapSearch" ? "display:none" : "") +
      '" alt="Map Layers" title="Map Layers" onclick="var dvs=document.getElementById(\'lyrs\').style;if(dvs.display==\'none\') dvs.display=\'inline\';else dvs.display=\'none\';" ><a><i class="ti ti-layers-alt"></i></a><ul class="ft-layer-link" id="lyrs" style="display:none;" onmouseleave="this.style.display=\'none\'"><li class="layer_tab" id="m_t' +
      MapmyIndia.num +
      '" onclick="MapmyIndia.tls(map_o[' +
      MapmyIndia.num +
      "]," +
      ...
      ",'','" +
      tileUrl +
      '\')" style="display:none"><a><span class="layer-item-img"><img src="https://maps.mapmyindia.com/images/mapview.jpg" style="width:50px !important"></span>Map</a></li>' +
      indic +
      satellite;
    if (params.traffic !== false && mmi[5].search("Traffic") != -1)
      innerHTML +=
        '<li id="t_t' +
        MapmyIndia.num +
        '" onclick=" MapmyIndia.tr(map_o[' +
        MapmyIndia.num +
        "]," +
        MapmyIndia.num +
        ')"><a><span class="layer-item-img"><img src="https://maps.mapmyindia.com/images/traffic.png" style="width:50px !important;"></span><span id="tr_t' +
        MapmyIndia.num +
        '">Traffic</span></a></li>';
    innerHTML += "</ul></li></ul></div>";
    element.innerHTML = innerHTML;
    if (id.id !== undefined) id = id.id;
    document.getElementById(id).appendChild(element);
    L.DomEvent.disableClickPropagation(element);
    var dis = MapmyIndia.num;
    try {
      var ptl = localStorage.getItem("tile");
      if (ptl) {
        defTile = ptl;
      }
    } catch (e) {}
    MapmyIndia.tls(map_o[MapmyIndia.num], MapmyIndia.num, defTile, tileUrl);
    if (params.search !== false && mmi[5].search("MapSearch") != -1) {
      var search_dv = L.control({ position: "topright" });
      var cls = "search_dv";
      if (params.location === false) var cls = "cur";
      search_dv.onAdd = function(map_o) {
        var div = L.DomUtil.create("div", cls);
        L.DomEvent.disableClickPropagation(div);
        div.innerHTML =
          "<div class='srch_dv'><table ><tr><td class='m_dvs' id='m_dv" +
          MapmyIndia.num +
          "' style='display:none'><input type='text' class='m_aus' id='m_auto" +
          MapmyIndia.num +
          "' placeholder='Search places or eLoc..' onkeyUp=\"MapmyIndia.auto('map_o[" +
          MapmyIndia.num +
          "]'," +
          MapmyIndia.num +
          ",'" +
          id +
          "','','','','')\" onclick=\"document.getElementById('m_result" +
          MapmyIndia.num +
          "').style.display='inline';\" autofocus><input type='button' class='m_clss' id='m_cls" +
          MapmyIndia.num +
          "' value='X' onclick=\"var ma=document.getElementById('m_auto" +
          MapmyIndia.num +
          "');if(!ma.value){document.getElementById('m_dv" +
          MapmyIndia.num +
          "').style.display='none';return false}ma.value='';ma.focus(); if(MapmyIndia.m_marker) map_o[" +
          MapmyIndia.num +
          "].removeLayer(MapmyIndia.m_marker);document.getElementById('m_result" +
          MapmyIndia.num +
          "').innerHTML='';document.getElementById('bt_srch" +
          MapmyIndia.num +
          "').src='" +
          MapmyIndia.s_icn +
          "';\"><div class='m_results' id='m_result" +
          MapmyIndia.num +
          "'></div></td><td title='Search to re center map' width='20px' class='ms_icon' valign='middle' onclick=\"MapmyIndia.auto('map_o[" +
          MapmyIndia.num +
          "]'," +
          MapmyIndia.num +
          ",'" +
          id +
          "','','','','')\"><img height=20 id='bt_srch" +
          MapmyIndia.num +
          "' src='https://maps.mapmyindia.com/images/srarch1.png'></td></tr></table></div>";
        return div;
      };
      search_dv.addTo(map_o[MapmyIndia.num]);
      map_o[MapmyIndia.num].on("click", function() {
        document.getElementById("lyrs").style.display = "none";
        document.getElementById("m_result" + dis).style.display = "none";
      });
    }
    MapmyIndia.num++;
    return map_o[dis];
    window.setTimeout(function() {
      map_o[MapmyIndia.num].invalidateSize();
    }, 2000);
  },
  mmi_t: [],
  bh_t: [],
  ...
  label: "",
  trf: "",
  sub: ["mt0", "mt1", "mt2", "mt3", "mt4", "mt5"],
  c_tile: "",
  tls: function(mpd, cnts, tile, tileUrl) {
    var att =
      "<img src='" +
      mmi[4].slice(0, -1) +
      "" +
      (!tile || tile == "dft" || tile == "reg"
        ? "../mmilogo"
        : "../watermark_hybrid") +
      ".png' style='width:100px !important'><br><span>Map data &#169;" +
      new Date().getFullYear() +
      " <a href='https://maps.mapmyindia.com' target='blank' style='color:#333'>MapmyIndia</a>";
    try {
      document.getElementById(
        (MapmyIndia.c_tile && MapmyIndia.c_tile != "dft"
          ? MapmyIndia.c_tile
          : "m") +
          "_t" +
          cnts
    ...
    document.getElementById(
      (tile && tile != "dft" ? tile : "m") + "_t" + cnts
    ).style.display = "none";
    mxzm = 19;
    MapmyIndia.c_tile = tile;
    try {
      document.querySelector(".myclass").style.display = "block";
    } catch (e) {}
    if (tile == "" || tile == "dft") {
      try {
        if (MapmyIndia.bh_t[cnts])
          map_o[cnts].removeLayer(MapmyIndia.bh_t[cnts]);
        if (MapmyIndia.hyb) map_o[cnts].removeLayer(MapmyIndia.hyb);
        if (MapmyIndia.label) map_o[cnts].removeLayer(MapmyIndia.label);
      } catch (e) {}
      if (map_o[cnts].hasLayer(MapmyIndia.mmi_t[cnts]))
        map_o[cnts].removeLayer(MapmyIndia.mmi_t[cnts]);
      MapmyIndia.mmi_t[cnts] = new L.tileLayer(
        tileUrl +
          "/advancedmaps/v1/" +
          map_key +
          "/" +
          (L.Browser.retina ? "retina_map" : "still_map") +
          "/{z}/{x}/{y}.png",
        {
          attribution: " ",
          maxZoom: mxzm,
          minZoom: mizm,
          subdomains: MapmyIndia.sub
        }
      );
      map_o[cnts].addLayer(MapmyIndia.mmi_t[cnts]);
      try {
        document.getElementById("attr").innerHTML =
          att +
          "<a style='padding-left:10px;cursor:pointer;color:#555' onclick=\"var url='https://maps.mapmyindia.com/report@'+btoa(map_o[" +
          cnts +
          "].getCenter().lat)+','+btoa(map_o[" +
          cnts +
          "].getCenter().lng)+','+btoa(map_o[" +
          cnts +
          "].getZoom());window.open(url);\" target='_blank' ><b>&#x02A36; Report</b></a></span>";
      } catch (e) {}
      if (map_o[cnts].hasLayer(MapmyIndia.trf)) {
        document.getElementById("tr_t" + cnts).innerHTML = "Traffic";
        MapmyIndia.tr(map_o[cnts], cnts);
      }
    } else if (tile == "reg") {
      try {
        if (MapmyIndia.bh_t[cnts])
      ...
      if (map_o[cnts].getZoom() > 17) map_o[cnts].setZoom(17);
      if (MapmyIndia.bh_t[cnts]) map_o[cnts].removeLayer(MapmyIndia.bh_t[cnts]);
      if (MapmyIndia.hyb) map_o[cnts].removeLayer(MapmyIndia.hyb);
      if (MapmyIndia.label) map_o[cnts].removeLayer(MapmyIndia.label);
      if (MapmyIndia.mmi_t[cnts])
        map_o[cnts].removeLayer(MapmyIndia.mmi_t[cnts]);
      MapmyIndia.mmi_t[
        cnts
      ] = new L.tileLayer(
        "https://{s}.mapmyindia.com/advancedmaps/v1/" +
          map_key +
          "/indic_tile/{z}/{x}/{y}.png",
        {
          attribution: " ",
          maxZoom: 17,
          minZoom: mizm,
      ...
      map_o[cnts].addLayer(MapmyIndia.mmi_t[cnts]);
      document.getElementById("attr").innerHTML = att.replace(
        "</a>",
        "</a>, Imagery &#169; Bhuvan NRSC"
      );
      if (map_o[cnts].hasLayer(MapmyIndia.trf)) {
        document.getElementById("tr_t" + cnts).innerHTML = "Traffic";
        MapmyIndia.tr(map_o[cnts], cnts);
      ...
      if (MapmyIndia.mmi_t[cnts])
        map_o[cnts].removeLayer(MapmyIndia.mmi_t[cnts]);
      if (MapmyIndia.hyb) map_o[cnts].removeLayer(MapmyIndia.hyb);
      MapmyIndia.bh_t[cnts] = new L.tileLayer(
        (mmi[11] ? mmi[11] : tileUrl) +
          "/advancedmaps/v1/" +
          map_key +
          "/bhuvan_imagery/{z}/{x}/{y}.png",
        {
          attribution: " ",
          maxZoom: 17,
          minZoom: mizm,
      ...
      MapmyIndia.hyb = new L.tileLayer(
        (mmi[11] ? mmi[11] : tileUrl) +
          "/advancedmaps/v1/" +
          map_key +
          "/" +
          (L.Browser.retina ? "retina_hybrid" : "base_hybrid") +
          "/{z}/{x}/{y}.png",
        {
          maxZoom: 17,
          minZoom: mizm,
      ...
      MapmyIndia.label = new L.tileLayer(
        (mmi[11] ? mmi[11] : tileUrl) +
          "/advancedmaps/v1/" +
          map_key +
          "/" +
          (L.Browser.retina ? "hybrid_lablel_retina" : "hybrid_label") +
          "/{z}/{x}/{y}.png",
        {
          maxZoom: 17,
          minZoom: mizm,
      ...
      map_o[cnts].addLayer(MapmyIndia.bh_t[cnts]);
      map_o[cnts].addLayer(MapmyIndia.hyb);
      map_o[cnts].addLayer(MapmyIndia.label);
      MapmyIndia.hyb.bringToFront();
      document.getElementById("attr").innerHTML = att.replace(
        "</a>",
        "</a>, Imagery &#169; Bhuvan NRSC"
      );
  ...
  tr: function(mpd, cnts) {
    var trft = document.getElementById("tr_t" + cnts);
    var maxzm = 19;
    if (MapmyIndia.c_tile !== "" && MapmyIndia.c_tile !== "dft") maxzm = 17;
    if (trft.innerHTML == "Traffic") {
      if (MapmyIndia.trf) map_o[cnts].removeLayer(MapmyIndia.trf);
      trft.innerHTML = "<b>&#9745; Traffic</b>";
      MapmyIndia.trf = new L.tileLayer(
        "https://{s}.mapmyindia.com/advancedmaps/v1/" +
          map_key +
          "/traffic_tile/{z}/{x}/{y}.png",
        { maxZoom: maxzm, minZoom: mizm, subdomains: MapmyIndia.sub }
      );
      mpd.addLayer(MapmyIndia.trf);
      MapmyIndia.trf.setZIndex(11112);
    } else {
      map_o[cnts].removeLayer(MapmyIndia.trf);
      trft.innerHTML = "Traffic";
    }
  },
  geo: function(mpp) {
    navigator.geolocation.getCurrentPosition(
      function(position) {
        var btn = L.divIcon({
          className: "",
          html:
            "<img src='https://maps.mapmyindia.com/images/current_location.png' height='30px'>"
        });
        if (cr) mpp.removeLayer(cr);
        var cr = L.marker(
      ...
      function(error) {
        alert("MapmyIndia does not have permission to view your location.");
      }
    );
  },
  req: "",
  pval: "",
  auto: function(mps, cnt, id, poi, lat, lng, eloc) {
    if (lat && lng) {
      var xhttp;
      ...
      xhttp.open(
        "GET",
        "https://apis.mapmyindia.com/map_v3/?lat=" + lat + "&lng=" + lng,
        true
      );
      xhttp.send();
      return false;
    }
    var did = document.getElementById("m_dv" + cnt);
    var vid = document.getElementById("m_auto" + cnt);
    if (did.style.display == "none") {
      did.style.display = "inline";
      vid.select();
    }
    var dsp = "",
      wd = document.getElementById(id).clientWidth;
    if (wd < 400) {
      /*dsp="document.getElementById('m_dv"+cnt+"').style.display='none';";*/
    }
    var val = vid.value;
    if (val.length > 2 && val.toLowerCase() != MapmyIndia.pval.toLowerCase()) {
      if (MapmyIndia.req) {
        MapmyIndia.req.abort();
        document.getElementById("bt_srch" + cnt).src = MapmyIndia.s_icn;
      }
      MapmyIndia.req = new XMLHttpRequest();
      if ("withCredentials" in MapmyIndia.req) {
        document.getElementById("bt_srch" + cnt).src =
          "https://maps.mapmyindia.com/images/load.gif";
        MapmyIndia.req.open(
          "GET",
        ...
        MapmyIndia.req.onreadystatechange = function() {
          if (
            MapmyIndia.req.status >= 200 &&
            MapmyIndia.req.status < 400 &&
            MapmyIndia.req.readyState === 4
          ) {
            var res = MapmyIndia.req.responseText;
            if (res) {
              var json = JSON.parse(res);
              var json_arr = json.results;
              if (!json_arr) return false;
              var div = "<ul class='m_ress" + cnt + " m_resu'>";
              MapmyIndia.pval = val;
              document.getElementById("m_result" + cnt).style.display =
                "inline";
              if (json.responseCode == 201)
                div += "<li> &#x27BD; No data found</li></ul>";
              else if (json.responseCode == 200) {
                for (var i = 0; i < json_arr.length; i++) {
                  var cl = "m_a_img";
                  var x = json_arr[i].x;
                  var y = json_arr[i].y;
                  var eloc = json_arr[i].elc;
                  var method = "MapmyIndia.poi";
                  if (!json_arr[i].user) {
                    var cl = "m_img";
                    method = "MapmyIndia.auto";
                  }
                  div +=
                    '<li onclick="' +
                    method +
                    ...
                    cl +
                    "'></div><b>" +
                    json_arr[i].addr.replace(",", "</b></br>") +
                    "</li>";
                }
              } else div = "";
              document.getElementById("m_result" + cnt).innerHTML = div;
              document.getElementById("bt_srch" + cnt).src = MapmyIndia.s_icn;
            }
          }
        };
    ...
    } else if (val.toLowerCase() == MapmyIndia.pval.toLowerCase())
      document.getElementById("m_result" + cnt).style.display = "inline";
    return false;
  },
  m_marker: "",
  poi: function(mps, cnt, poi, lat, lng, elc) {
    document.getElementById("m_auto" + cnt).value = poi.split(",")[0];
    MapmyIndia.pval = poi.split(",")[0];
    document.getElementById("m_result" + cnt).style.display = "none";
    document.getElementById("m_dv" + cnt).style.display = "none";
    html =
      "<table width='200px' style='font-size:13px'><tr><td><b>" +
      poi
        .replace(",", "</b><br>")
        .replace("</b><br>", "</b></td><tr><td colspan=2>") +
      "<br>eLoc:</b> " +
      elc +
      "</td></tr></table>";
    if (MapmyIndia.m_marker) map_o[cnt].removeLayer(MapmyIndia.m_marker);
    MapmyIndia.m_marker = new L.Marker(new L.LatLng(lng, lat)).bindPopup(html, {
      minWidth: "120px",
      maxWidth: "120px"
    });
    MapmyIndia.m_marker.addTo(map_o[cnt]);
    setTimeout(function() {
      map_o[cnt].setView(new L.LatLng(lng, lat), 16);
      MapmyIndia.m_marker.openPopup();
    }, 100);
  },
  kml: function(map_obj, file, options) {
    var kmlLayer = new L.KML(file, options);
    try {
      map_obj.addLayer(kmlLayer);
    } catch (e) {
      console.log(
    ...
    var ftb = false;
    try {
      ftb = options.fitBounds;
    } catch (e) {}
    if (ftb) {
      if (file.substr(-3) == "kml") {
        kmlLayer.on("loaded", function(e) {
          map_obj.fitBounds(kmlLayer.getBounds());
        });
      } else map_obj.fitBounds(kmlLayer.getBounds());
    }
    return kmlLayer;
  }
};
